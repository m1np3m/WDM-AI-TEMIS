...
def parse_args():
    ...
    parser.add_argument("--force_create", action="store_true", help="Force recreate collection even if it exists")
    return parser.parse_args()

class Args:
    def __init__(self, pdf_folder, qa_path, chunk_size, chunk_overlap, num_docs, embedding_model_name, chunk_type, hybrid_search, reranker_model_name, path_to_save, force_create=False):
...
async def main(args):
    ...
    embedding_name = embedding_model_name.split('/')[-1] if '/' in embedding_model_name else embedding_model_name
    ...
    if global_documents["page_documents"] == [] or global_documents["table_documents"] == []:
        all_page_documents, all_table_documents = await process_all_pdfs_in_folder(pdf_folder, qa_path)
    ...
    try:
        ...
        try:
            if client.collection_exists(COLLECTION_NAME):
                client.delete_collection(COLLECTION_NAME)
                print(f"✅ Successfully deleted incomplete collection {COLLECTION_NAME}")
    ...
    # run evaluation
    doc_retrieval_function = exp.get_documents if not hybrid_search else exp.get_documents_hybrid
    reranker_function = None if not reranker_model_name else Reranker(method=reranker_model_name)    
...
if __name__ == "__main__":
    ...
    args = Args(
        ...
        force_create=False) # for testing
