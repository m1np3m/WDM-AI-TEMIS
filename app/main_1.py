...
@st.cache_resource
def initialize_rag(embedding_type, embedding_model, enable_hybrid_search, chunk_type, collection_name, persist_dir):
...
def main():
    ...
    with st.sidebar:
        ...
        if ("rag" not in st.session_state or 
            st.session_state.rag is None or 
            "rag_config_key" not in st.session_state or 
                        ...
                        if all_splits:
                            ...
                            try:
                                with st.spinner("Adding documents to vector database..."):
                                    st.session_state.rag.add_documents(documents=all_splits)
                    ...
                    if st.session_state.rag:
                        ...
                        with context_col:
                            ...
                            if docs:
                                ...
                                if st.checkbox("🔍 Show Full Context", key=f"show_context_{len(st.session_state.messages)}"):
