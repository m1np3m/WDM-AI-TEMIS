...
class ColPaliModel:
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            raise ValueError(
               "Incorrect model name specified. This code only supports ColPali and ColQwen2 for now."
            )
        ...
        if not load_from_index:
            ...
            with open(index_path / "embed_id_to_doc_id.json", 'r') as f:
                self.embed_id_to_doc_id = json.load(f)
            ...
            try:
                with open(index_path / "doc_ids_to_file_names.json", 'r') as f:
                    self.doc_ids_to_file_names = json.load(f)
                self.doc_ids_to_file_names = {
            ...
            if metadata_path.exists():
                with open(metadata_path, 'r') as f:
                    self.doc_id_to_metadata = json.load(f)
    ...
    def _export_index(self):
        ...
        with open(index_path / "index_config.json", 'w') as f:
            json.dump(index_config, f)

        # Save embed_id_to_doc_id mapping
        with open(index_path / "embed_id_to_doc_id.json", 'w') as f:
            json.dump(self.embed_id_to_doc_id, f)

        # Save doc_ids_to_file_names
        with open(index_path / "doc_ids_to_file_names.json", 'w') as f:
            json.dump(self.doc_ids_to_file_names, f)

        # Save metadata
        with open(index_path / "metadata.json", 'w') as f:
            json.dump(self.doc_id_to_metadata, f)
        ...
        if self.full_document_collection:
            ...
            for i in range(0, len(self.collection), 500):
                ...
                with open(collection_path / f"{i}.json", 'w') as f:
                    json.dump(chunk, f)

        if self.verbose > 0:
            logger.info(f"Index exported to {index_path}")
    ...
    def index(
        ...
        if input_path.is_dir():
            ...
            for i, item in enumerate(items):
                logger.info(f"Indexing file: {item}")
    ...
    def add_to_index(
        self,
        input_item: Union[str, Path, List[Union[str, Path]]],
        ...
        if isinstance(input_item, (str, Path)) and Path(input_item).is_dir():
            input_items = [item for item in Path(input_item).iterdir() if item.suffix.lower() == ".pdf"]
        else:
            input_items = (
                [input_item] if not isinstance(input_item, list) else input_item
            )

        # Validate that all inputs are PDF files
        for item in input_items:
            if isinstance(item, (str, Path)):
                if not Path(item).suffix.lower() == ".pdf":
                    raise ValueError(f"Only PDF files are supported. Found: {item}")
            else:
                raise ValueError(f"Only PDF files are supported. Found: {type(item)}")
    ...
    def _process_directory(
        ...
        pdf_files = [item for item in directory.iterdir() if item.suffix.lower() == ".pdf"]
    ...
    def _process_and_add_to_index(
        ...
        if not item.suffix.lower() == ".pdf":
            raise ValueError(f"Only PDF files are supported. Found: {item.suffix}")
    ...
    def _add_to_index(
        ...
        if self.verbose > 0:
            logger.info(f"Added page {page_id} of document {doc_id} to index.")
